...
docker:
	docker buildx build --build-arg='BUILD_TAGS=mainnet,experimental' -t tztcloud/go-livepeer:open-pool-0.7.5 -f docker/Dockerfile .

docker-windows:
	docker buildx build --build-arg='BUILD_TAGS=mainnet,experimental' -t open-pool:0.7.5-windows-binary -f docker/Dockerfile.window-build .

docker-linux:
	docker buildx build --build-arg='BUILD_TAGS=mainnet,experimental' -t open-pool:0.7.5-linux-binary -f docker/Dockerfile.linux-build .